...
def write_and_read_txt(filename):
    with open(filename, 'r') as file :
        values= [str(line.strip()) for line in file]
        return np.array(values)
    return np.array([])
print(write_and_read_txt(r"C:\Users\6292492\Desktop\assigmnet 3\numbers.txt"))
    

# Function 2: File I/O - Writing and Reading from a CSV File
# Writes a list of lists to a CSV file and reads it back.
def write_and_read_csv(filename):
    with open(filename, 'r') as file :
        values= [float(line.strip()) for line in file]
        return np.array(values)
    return np.array([])
print( write_and_read_csv(filename)("C:\Users\6292492\Desktop\assigmnet 3\curve.csv"))
